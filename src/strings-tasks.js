...
function removeFirstOccurrences(str, value) {
  ...
  return start !== -1
    ? str.substring(0, start) + str.substring(start + value.length)
    : str;
}
...
function formatTime(minutes, seconds) {
  const min = minutes / 1 <= 10 ? `0${minutes}` : minutes;
  const sec = seconds / 1 <= 10 ? `0${seconds}` : seconds;
  return `${min}:${sec}`;
}
...
function countVowels(str) {
  ...
  return Array.from(str).filter((value) =>
    symbols.includes(value.toLowerCase())
  ).length;
}
...
function isPalindrome(str) {
  const replaced = str.toLowerCase().replace(/\W+|\d+/g, '');
...
function findLongestWord(sentence) {
  const sorted = sentence.split(' ').sort((a, b) => b.length - a.length);
  return sorted[0];
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((part) => Array.from(part).reverse().join(''))
    .join(' ');
}
